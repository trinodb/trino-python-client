...
def _require_sub(
    pattern: str, replacement: str | Callable[[re.Match[str]], str], string: str, label: str, **kwargs: Any
) -> str:
    """Like re.sub(), but exits with an error when the pattern does not match."""
    result, count = re.subn(pattern, replacement, string, **kwargs)
    if count == 0:
        print(f"ERROR: no match for {label} pattern: {pattern}", file=sys.stderr)
        sys.exit(1)
    return result
...
def partition_versions(cycle: dict[str, Any]) -> tuple[set[str], set[str]]:
...
def update_setup_py(content: str, to_add: set[str], to_remove: set[str], min_active: str) -> str:
    ...
    for version in sorted(to_remove, key=version_key):
        content = re.sub(prefix + re.escape(version) + r'",\n', "", content)
    ...
    if to_add:
        ...
        content = _require_sub(
            rf'({prefix}Implementation :: CPython")',
            new_lines + r"\1",
            content,
...
def update_tox_ini(content: str, active_versions: set[str]) -> str:
    envlist = ",".join(
        "py" + version.replace(".", "")
        for version in sorted(active_versions, key=version_key)
    )
    return _require_sub(r"^envlist = .*$", f"envlist = {envlist}", content, "tox.ini envlist", flags=re.MULTILINE)


def _extract_yaml_block(content: str, key: str) -> tuple[int, int]:
    """Return (start, end) offsets of the child block under a YAML key.
    Finds ``key:`` and collects all subsequent lines indented deeper than it.
    """
    ...
    for line in content[block_start:].splitlines(keepends=True):
...
def update_ci_yml(content: str, active_versions: set[str], eol_versions: set[str], latest_stable: str) -> str:
    ...
    def rebuild_python_list(match: re.Match[str]) -> str:
        existing_pypy = re.findall(r'"pypy-(3\.\d+)"', match.group(2))
        valid_pypy = list(dict.fromkeys(version for version in existing_pypy if version not in eol_versions))
        lines = [f'          "{version}",' for version in sorted_versions]
        lines += [f'          "pypy-{version}",' for version in valid_pypy]
    ...
    # Update include entries - scoped to the include block via indentation
    block_start, block_end = _extract_yaml_block(content, "include")
    if block_start >= 0:
        block = content[block_start:block_end]
        include_versions = re.findall(r'python: "(\d+\.\d+)"', block)
        if include_versions:
            old_include_version = max(include_versions, key=version_key)
...
def main() -> None:
    ...
    eol, active = partition_versions(cycle)
    if not active:
    ...
    files: dict[str, Callable[[str], str]] = {
        "setup.py": lambda content: update_setup_py(content, to_add, to_remove, min_active),
        "tox.ini": lambda content: update_tox_ini(content, active),
        ".github/workflows/ci.yml": lambda content: update_ci_yml(content, active, eol, latest_stable),
        ".github/workflows/release.yml": lambda content: update_release_yml(content, latest_stable),
        "README.md": lambda content: update_readme(content, min_active),
        ".pre-commit-config.yaml": lambda content: update_pre_commit(content, min_active),
    }
