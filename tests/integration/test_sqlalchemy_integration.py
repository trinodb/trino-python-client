...
def trino_connection(run_trino, request):
    ...
    engine = sqla.create_engine(f"trino://test@{host}:{port}/{request.param}", connect_args=connect_args)
...
@pytest.mark.skipif(sqlalchemy_version() < "2.0", reason="sqlalchemy.Uuid only exists with SQLAlchemy 2.0 and above")
@pytest.mark.parametrize("trino_connection", ["memory"], indirect=True)
...
def test_insert_uuid(trino_connection):
    ...
    metadata = sqla.MetaData()
    try:
        users = sqla.Table("users", metadata, sqla.Column("guid", sqla.Uuid), schema="test")
        metadata.create_all(engine)
        ins = users.insert()
        guid = uuid.uuid4()
...
def test_json_column_operations(trino_connection):
    ...
    json_object = {"a": {"c": 1}, 100: {"z": 200}, "b": 2, 10: 20, "foo-bar": {"z": 200}}
