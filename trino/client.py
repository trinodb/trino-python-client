    ...
    def fetch(self) -> List[Union[List[Any]], Any]:
        ...
        if isinstance(status.rows, dict):
            ...
            if self._fetch_mode == "segments":
                return segments
            return list(SegmentIterator(segments, self._request, self._row_mapper))
...
class Segment(abc.ABC):
    ...
    Attributes:
        ...
        rows (property): Returns the decoded and mapped rows of data.
    """
...
class InlineSegment(Segment):
    ...
    Attributes:
        rows (property): The rows of data in the segment, decoded and mapped from the base64 encoded data.
    """
...
class SpooledSegment(Segment):
    ...
    Attributes:
        rows (property): The rows of data, loaded and mapped from the spooled segment.
    ...
    def _send_spooling_request(self, uri: str, segment: _SpooledSegmentTO) -> requests.Response:
        ...
        for key, values in segment.get("headers", {}).items():
            if len(values) > 1:
                raise ValueError(f"Header '{key}' contains multiple values: {values}")
            headers_with_single_value[key] = values[0]
        return self._request._get(uri, headers=headers_with_single_value)
...
class SegmentIterator:
    def __init__(self, spooled_data: SpooledData, request: TrinoRequest, mapper: RowMapper) -> None:
        self._segments = iter(spooled_data._segments)
        self._decoder = SegmentDecoder(CompressedQueryDataDecoderFactory(mapper).create(spooled_data.encoding), request)
        self._decoder = SegmentDecoder(CompressedQueryDataDecoderFactory(mapper).create(spooled_data.encoding), request)
...
class SegmentDecoder():
    def __init__(self, decoder: QueryDataDecoder, request: TrinoRequest):
...
class CompressedQueryDataDecoder(QueryDataDecoder):
    ...
    def decode(self, data: bytes, metadata: _SegmentMetadataTO) -> List[List[Any]]:
        if "uncompressedSize" in metadata:
            # Data is compressed
